...
# Working in Jev Atlas

The app lives at the repo root. Read `AGENTS.md` before changing anything: it is the worker
...
Also busy when 4+ Claude workers are in **Running
...
In `AGENTS.md` (imported above) — they bind Jarvis as much as workers.

## Checks

Workers run these before handing off and report the results: `npm run check` (typecheck, tests, production build).
